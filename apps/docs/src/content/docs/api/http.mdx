...
```http
GET /api/tabula-lens?table=users
```

#### Paginated Query

```http
GET /api/tabula-lens?table=users&page=2&pageSize=25
```

#### Filtered Query

```http
GET /api/tabula-lens?table=users&filter=john
```

#### Filtered Query with Specific Columns

```http
GET /api/tabula-lens?table=users&filter=john&filterColumns=name,email
```

#### Sorted Query

```http
GET /api/tabula-lens?table=users&sort=created_at:desc
```

#### Multi-column Sort

```http
GET /api/tabula-lens?table=users&sort=status:asc,created_at:desc
```

#### Column Selection

```http
GET /api/tabula-lens?table=users&columns=id,name,email
```

#### Complete Query with All Parameters

```http
GET /api/tabula-lens?table=users&page=1&pageSize=25&filter=john&filterColumns=name,email&sort=created_at:desc&columns=id,name,email,created_at
...
The `pageSize` parameter specifies how many records to return per page.
...
```
cache_key = "tabula_lens:{table}:{page}:{pageSize}:{filter}:{filterColumns}:{sort}:{columns}"
```
...
- **Database**: Any database can be supported (PostgreSQL is the primary supported database)
...
```bash
# Basic query
curl "http://localhost:3000/api/tabula-lens?table=users"

# With authentication
curl -H "Authorization: Bearer your-token" \
  "http://localhost:3000/api/tabula-lens?table=users&page=1&pageSize=25"

# With filtering and sorting
curl "http://localhost:3000/api/tabula-lens?table=users&filter=john&sort=created_at:desc"
```
...
```javascript
// Basic query
const response = await fetch('http://localhost:3000/api/tabula-lens?table=users');
...
const response = await fetch(`http://localhost:3000/api/tabula-lens?${params}`);
const data = await response.json();
```
...
// Basic query
const response = await axios.get('http://localhost:3000/api/tabula-lens', {
  params: { table: 'users' }
});

// With authentication
const response = await axios.get('http://localhost:3000/api/tabula-lens', {
  params: { table: 'users', page: 1, pageSize: 25 },
...
# Basic query
response = requests.get('http://localhost:3000/api/tabula-lens', 
                       params={'table': 'users'})
data = response.json()
...
response = requests.get('http://localhost:3000/api/tabula-lens', params=params)
data = response.json()
```
