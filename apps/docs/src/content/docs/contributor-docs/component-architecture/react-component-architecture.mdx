...
function MyComponent() {
  ...
  return <div>Current table: {selectedTable}</div>;
}
```
...
function MyComponent() {
  const { selectedTable, pagination, sorting, debouncedFilter } = useTableState({
    initialTable: 'users',
  });
...
Builds a URL query string from table state.

**Features:**
- Type-safe parameter building using TanStack Table types
- Handles sorting state (array of `{ id, desc }` objects)
- Handles pagination state (`{ pageIndex, pageSize }`)
- URL-encodes filter and sort values
...
// Result: "table=users&page=1&limit=20&sort=id%3Aasc&filter=john&filterColumns=name%2Cemail"
...
const isValid = validatePagination({ page: 1, limit: 20, total: 100, totalPages: 5 });
