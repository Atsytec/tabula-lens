...
- **MySQL**: Not currently implemented
...
TabulaLens implements efficient connection pooling via the underlying Knex.js and `pg` driver. Connection pool settings are controlled through the PostgreSQL connection string parameters or `pg` environment variables, not through TabulaLens constructor options:

```bash
# Pool size via connection string
